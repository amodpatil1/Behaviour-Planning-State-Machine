...
class FSMNode(Node):
    def __init__(self, blackboard):
        ...
        self.stop_drive_publisher = self.create_publisher(Bool, '/stop_drive', 10)
...
class StopWhenObstacleDetectedState(State):
    ...
    def execute(self, blackboard: Blackboard) -> str:
        ...
        # Publish to /stop_drive topic
        logging.info("Publishing to /stop_drive topic")
        stop_drive_msg = Bool()
...
class ParkingState(State):
    def __init__(self, fsm_node) -> None:
        super().__init__(["parked", "stop_when_obstacle_detected"])
        self.fsm_node = fsm_node
