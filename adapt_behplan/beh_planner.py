...
class CarBehaviour(Node):
    ...
    def steering_calc(self):
        if self.curr_way_idx < len(self.waypoints)-1:
            ...
            if self.curr_yaw == -90.0:
                self.get_logger().info('no deviation')

            else:   
                if 0.0 < self.curr_yaw < -90.0:
                    self.dev_angle = 90.0 - abs(self.curr_yaw)
                    self.angular_vel =  float(self.dev_angle * 0.23)
                    self.get_logger().info('steering angle: %f' % (self.angular_vel))
                    return float(self.angular_vel)

                if -90.0 < self.curr_yaw < -180.0:
                    self.dev_angle = abs(self.curr_yaw) - 90.0
                    self.angular_vel = float(-self.dev_angle * 0.23)
                    self.get_logger().info('steering angle: %f' % (self.angular_vel))
                    return float(self.angular_vel) 
            return 0.0
